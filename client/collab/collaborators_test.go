...
var _ = Describe("Collaborators", func() {
	...
	DescribeTable("List",
		func(e expectation) {
			...
			collaborators, appErr := collab.List("t0k3n", "orient-express")
			Expect(server.ReceivedRequests()).To(HaveLen(1))

			if e.errIsNil {
				Expect(appErr).To(BeNil())
				Expect(collaborators).To(Equal(e.result))
			} else {
		...
		Entry("unexpected response code", expectation{
			...
			resBody:    "",
			errIsNil:   false,
			errCode:    collab.ErrCodeUnexpectedError,
			errDesc:    "",
			errIsFatal: true,
		}),
		...
		Entry("404 with project not found", expectation{
			...
			errIsNil:   false,
			errCode:    collab.ErrCodeNotFound,
	...
	DescribeTable("Add",
		func(e expectation) {
			...
			appErr := collab.Add("t0k3n", "orient-express", e.email)
			Expect(server.ReceivedRequests()).To(HaveLen(1))
		...
		Entry("409 Conflict", expectation{
			...
			errIsNil:   false,
			errCode:    collab.ErrCodeAlreadyExists,
		...
		Entry("422 with email is not found", expectation{
			...
			errIsNil:   false,
			errCode:    collab.ErrCodeUserNotFound,
			errDesc:    "",
			errIsFatal: true,
		}),
		...
		Entry("422 with owner cannot be added", expectation{
			...
			errIsNil:   false,
			errCode:    collab.ErrCodeCannotAddOwner,
			errDesc:    "",
			errIsFatal: true,
		}),
	...
	DescribeTable("Remove",
		func(e expectation) {
			...
			appErr := collab.Remove("t0k3n", "orient-express", e.email)
			Expect(server.ReceivedRequests()).To(HaveLen(1))
